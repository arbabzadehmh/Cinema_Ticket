                    ...
                    <c:forEach var="customer" items="${sessionScope.allCustomers}">
                        ...
                        <tr>
                            ...
                            <td>
                                ...
                                <c:choose>
                                    <c:when test="${customer.imageUrl != ''}">
                                        <img src="${customer.imageUrl}" alt="Manager Image" height="80px" width="80px">
                                    </c:when>
                                    <c:otherwise>
                                        No Image
                                    </c:otherwise>
...
<script>
            ...
            success: function (response) {
                ...
                if (Array.isArray(response) && response.length > 0) {
                    response.forEach(function (customer) {
                        var imageHtml = customer.imageUrl ? "<img src='" + customer.imageUrl + "' alt='Customer Image' height='80px' width='80px'>" : "No Image";
    ...
    function findCustomerByPhoneNumber(phoneNumber) {
        ...
        $.ajax({
            url: "/rest/customer/findByPhone/" + phoneNumber,
            method: "GET",
            dataType: "json", // Expect JSON response
            success: function (response) {
                // Clear previous results
